...
describe("SFV", () => {
	...
	it("parses string list", () => {
		assert.deepEqual(parseFetchurlServer('"https://a.com", "https://b.com"'), [
			"https://a.com",
			"https://b.com",
		]);
	});
	...
	it("roundtrips", () => {
		const urls = [
			"https://cdn.example.com/f.tar.gz",
			"https://mirror.org/a.tgz",
		];
...
describe("fetchurl()", () => {
	...
	it("falls back from failed server to direct source", async () => {
		...
		const bad = await startServer((_, res) => {
			res.writeHead(500);
			res.end();
		});
		const good = await startServer((_, res) => {
			res.writeHead(200);
			res.end(content);
		});
		try {
